...
class Cicciobello:
    ...
    def calculate_pp(self):
        try:
            ...
            print("ciccio structure", difficulty.__dict__)

            # Calculate pp
            if self.tillerino:
                results = []
                for acc in (1.00, 0.99, 0.98, 0.95):
            ...
            log.error(f"cicciobello ~> Unhandled exception: {e}")
            self.pp = 0
            raise
        finally:
            log.debug(f"cicciobello ~> Shutting down, pp = {self.pp}")
