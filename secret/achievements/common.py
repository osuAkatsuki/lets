...
def get_usercache(userID):
	user_cache = glob.redis.get(f"lets:user_achievement_cache:{userID}")
	if user_cache is None:
		user_cache = {}
	else:
		user_cache = orjson.loads(user_cache.decode("utf-8"))
...
def add_pending_achievement(userID, achievementID):
	...
	if len([x for x in user_cache["achievements"] if x in (achievementID, -achievementID)]) > 0:
		print(f"Tried to add achievement:{achievementID} to user:{userID}, but failed due to duplicate entry.")
		return
	...
	glob.redis.set(f"lets:user_achievement_cache:{userID}", orjson.dumps(user_cache), 1800)
